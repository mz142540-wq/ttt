 ...
 */
const scrUnitTag = (scrTag, unitLimit = 1700, currFrame) => {
  ...
  if (index >= 1700) {
    throw new Error(`1.16 replay unit limit reached, @Frame:${currFrame}, Unit:${scrTag} -> index:${index}`);
  }
  ...
  if (tag < 0)
  {
    console.log(`Frame:${currFrame}, Unit:${scrTag} -> ${tag} is < 0`);
    return scrTag;
  }
