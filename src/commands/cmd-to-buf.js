...
const commandToBuf = (cmdId, cmd, isRemastered, unitLimit = 1700, currFrame) => {
  ...
  switch (cmdId) {
    case CMDS.RIGHT_CLICK_EXT.id:
      return [
        ...
        new BufferList([
          uint16(cmd.x),
          uint16(cmd.y),
          uint16(scrUnitTag(cmd.unitTag, unitLimit, currFrame)),
          uint16(cmd.unit),
          uint8(cmd.queued),
        ]),
      ];
    ...
    case CMDS.SELECTION_REMOVE_EXT.id: {
      ...
      return [
        ...
        new BufferList([
          ...
          ...cmd.unitTags.map((tag) => uint16(scrUnitTag(tag, unitLimit, currFrame))),
        ]),
      ];
    }
    ...
    case CMDS.UNLOAD_EXT.id:
      return [CMDS.UNLOAD.id, new BufferList([uint16(scrUnitTag(cmd.unitTag, unitLimit, currFrame))])];

    case CMDS.CANCEL_TRAIN.id:
      return [cmdId, new BufferList([uint16(scrUnitTag(cmd.unitTag, unitLimit, currFrame))])];
