...
const downgradeReplay = async (replay, chkDowngrader) => {
  ...
  // If newFrame != currFrame, flush everything accumulated in frameBuf to commandsBuf via dumpFrame, resets frameBuf.
  try {
    while ((cmd = g.next().value)) {
      // Frame boundary marker, if its not a cmd ?
      ...
      {
        console.log('cmd.skipped + !cmd.data', currFrame);
        continue;
      }
