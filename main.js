...
async function convertReplay(filePath, useMtxm) {
  ...
  fs.writeFileSync(filePath + '.original.chk', Buffer.from(srep.chk));
  console.log(`---> Saved original CHK: ${srep.chk.length} bytes`);

  const chkDowngrader = new downgrade.ChkDowngrader({ mtxm: useMtxm });
  const arr = await downgrade.downgradeReplay(srep, chkDowngrader);
